...
const imageTerms = [
  "Das Shooting erfolgt auf TFP-Basis. Eine Vergütung in Geld erfolgt nicht. Chris Franz Design entscheidet, welche Aufnahmen bearbeitet und dem Model als digitale Auswahl mit Wasserzeichen bereitgestellt werden. Aus dieser bearbeiteten Auswahl darf jedes Model als Gegenleistung für seine Mitwirkung genau fünf Fotos auswählen. Diese fünf ausgewählten Fotos werden anschließend ohne Wasserzeichen und ausschließlich digital übermittelt.",
  "Weitere vollständig bearbeitete Fotos ohne Wasserzeichen werden nur herausgegeben, wenn Chris Franz Design und das Model dies ausdrücklich und individuell vereinbaren. Die Bereitstellung kann unentgeltlich oder zu gesondert vereinbarten Bedingungen erfolgen. Ein Anspruch auf weitere Bilder besteht nicht. Auswahl und Bearbeitungsstil liegen bei Chris Franz Design. Unbearbeitete Aufnahmen und RAW-Dateien werden nicht herausgegeben.",
...
export default function Home() {
  ...
  const makePdf = async () => {
    ...
    imageTerms.forEach((paragraph) => {
      const termsLines = pdf.splitTextToSize(paragraph.replace(/Chris Franz Design/g, "CHRIS FRANZ DESIGN"), contentWidth);
      ensureSpace(termsLines.length * 3.8 + 5);
      pdf.text(termsLines, margin, y, { lineHeightFactor: 1.3 });
  ...
  return <main><section className="card">
    ...
    <form onSubmit={(event) => { event.preventDefault(); if (ready) makePdf(); }}>
      ...
      <fieldset><legend>Einwilligung und TFP-Vereinbarung</legend><p className="copy">{consentText.split("Chris Franz Design")[0]}<em className="brand-name">Chris Franz Design</em>{consentText.split("Chris Franz Design")[1]}</p><div className="terms-copy">{imageTerms.map((paragraph, index) => <p key={index}>{paragraph}</p>)}</div><label className="check"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} required /><span>Alle aufgeführten Models beziehungsweise ihre erziehungsberechtigten Personen haben die Vereinbarung gelesen und stimmen ihr zu. *</span></label></fieldset>
